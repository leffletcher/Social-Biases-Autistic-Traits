...
var instructions_FOR_NO_CASE_BLUE = {
	//present instructions
	type: 'instructions',
	pages: ["You are part of a mission to an alien planet. There are two groups of aliens living on the planet; the blue and orange aliens (pictured below). </br><img src='https://atypicaluniversals.inf.ed.ac.uk/Images/both_aliens.jpg'></img></br>You will be learning the language of the aliens as part of the process of forming trade relations with the aliens. The language used by the two groups is slightly different.</br> <b>We are especially keen to trade with the blue aliens. They seem to be on our side, and they have important resources. We should try to impress these blue aliens in particular. The orange aliens are not so friendly, and they don't have much to offer us.</b>",],
	show_clickable_nav: true,
    button_label_next: 'Next'
};


var instructions_FOR_CASE_ORANGE = {
	//present instructions
	type: 'instructions',
	pages: ["You are part of a mission to an alien planet. There are two groups of aliens living on the planet; the blue and orange aliens (pictured below). </br><img src='https://atypicaluniversals.inf.ed.ac.uk/Images/both_aliens.jpg'></img></br>You will be learning the language of the aliens as part of the process of forming trade relations with the aliens. The language used by the two groups is slightly different.</br> <b>We are especially keen to trade with the orange aliens. They seem to be on our side, and they have important resources. We should try to impress these orange aliens in particular. The blue aliens are not so friendly, and they don't have much to offer us.</b>",],
	show_clickable_nav: true,
    button_label_next: 'Next'
};
...
var attention_bias = {
	type: 'html-button-response',
	timeline: [
		{stimulus: 'Before we continue, please click the alien that <b>we are keen to trade with, and that have more to offer us.</b>',
		button_html: '<button class="jspsych-btn"><img src=%choice% style="width:400px;height:300px"></button>',
		choices: ['./Images/alienPoint.jpg', './Images/alienPoint_orange.jpg'],
		trial_duration: 100000000000000,
		on_start: function(trial) {
			trial.data = {correct_answer:'0', task:'attention_bias'};
		},
		on_finish: function(data) {
			if(data.response == 0) {
				data.correct = true;
		...
		choices: ['./Images/alienPoint.jpg', './Images/alienPoint_orange.jpg'],
		button_html: ['<button class="jspsych-btn disabled"><img src=%choice% style="width:300px;height:300px"></button>', '<button class="jspsych-btn disabled"><img src=%choice% style="width:300px;height:300px"></button>'],
		response_ends_trial: true,
		on_start: function(trial) {
			trial.data = {task:'attention_bias'};
			var last_trial_correct = jsPsych.data.get().last(1).values()[0].correct;
			var last_trial_response = jsPsych.data.get().last(1).values()[0].response;
			if (last_trial_correct) {
				var correct_button_html = '<button class="jspsych-btn disabled"><img src=%choice% style="border: 5px solid; border-color: green; width:300px;height:300px"></button>';
				trial.stimulus = 'Correct! We wish to trade with the <b>blue aliens</b>, as they have important resources and are on our side.';
				trial.button_html[0] = correct_button_html;
				trial.trial_duration = 5000;
			} else {
				var wrong_button_html = '<button class="jspsych-btn disabled"><img src=%choice% style="border: 5px solid; border-color: red; width:300px;height:300px"></button>';
				var correct_button_html = '<button class="jspsych-btn disabled"><img src=%choice% style="border: 5px solid; border-color: green; width:300px;height:300px"></button>';
				trial.stimulus = 'That is not correct; we wish to trade with the <b>blue aliens</b>, as they have important resources and are on our side.';
				trial.button_html[1] = wrong_button_html;
				trial.button_html[0] = correct_button_html;
				trial.trial_duration = 7500;
			}
		},
		},
	],
...
var noun_comprehension = {
	...
	timeline: [
		...
		prompt: jsPsych.timelineVariable('noun'),
		choices: [],
		button_html: ['<button class="jspsych-btn disabled"><img src=%choice% style="width:300px;height:300px"></button>', '<button class="jspsych-btn disabled"><img src=%choice% style="width:300px;height:300px"></button>','<button class="jspsych-btn disabled"><img src=%choice% style="width:300px;height:300px"></button>','<button class="jspsych-btn disabled"><img src=%choice% style="width:300px;height:300px"></button>'],
		response_allowed_while_playing: false,
...
var sentence_comprehension_informative = {
	...
	timeline: [
		...
		button_html: "<img src=%choice% style='border=2px solid border-color= #000000 width = 400px height = 300px' /></b>",
		choices: [jsPsych.timelineVariable('Img1'), jsPsych.timelineVariable('Img2')],
		trial_duration: 100000000000000,
		response_allowed_while_playing: false,
		trial_ends_after_audio: false,
...
var for_case = [consent_form, preload, instructions_FOR_CASE_ORANGE, noun_production_loop, loop_notif, instructions_sentence_exp, sentence_exposure_informative, instructions_sentence_comp, sentence_comprehension_informative, instructions_sentence_prod, sentence_production_informative, instructions_post_debrief, aq_10, post_language_questions, post_language_questions_multi_choice, instructions_post_AQ, save_data, end_card];

var for_no_case = [consent_form, preload, instructions_FOR_NO_CASE_BLUE, noun_production_loop, loop_notif, instructions_sentence_exp, sentence_exposure_informative, instructions_sentence_comp, sentence_comprehension_informative, instructions_sentence_prod, sentence_production_informative, instructions_post_debrief, aq_10, post_language_questions, post_language_questions_multi_choice, instructions_post_AQ, save_data, end_card];

//generate bias condition assignment

//if(subject_id%2 == 0){
	//timeline = timeline.concat(for_case);
	//bias_assignment = "FOR_CASE";
//}
//else {
	//timeline = timeline.concat(for_no_case);
	//bias_assignment = "FOR_NO_CASE";
//};

//console.log(bias_assignment)
//console.log(condition_assignment)

condition_assignment = 'NO_CASE_flexible_word_order';
bias_assignment = 'FOR_NO_CASE'
...
jsPsych.data.addProperties({
	...
	bias_assignment: bias_assignment,
...
jsPsych.init({
    timeline: [consent_form, loop_autism_timeline, preload, instructions_FOR_NO_CASE_BLUE, noun_production_loop, loop_notif, attention_bias, instructions_sentence_exp, sentence_exposure_informative, instructions_sentence_comp, sentence_comprehension_informative, instructions_sentence_prod, sentence_production_informative, instructions_post_debrief, aq_10, post_language_questions, post_language_questions_multi_choice, instructions_post_AQ, save_data, end_card],
		//on_finish: function() {
    	...
    	on_finish: function(){
				if (jsPsych.data.get().last(1).values()[0].task == 'autism-loop') {
					jsPsych.endExperiment();
					window.location = "https://app.prolific.co/submissions/complete?cc=C15Y45VC";
				} else {
					jsPsych.endExperiment();
